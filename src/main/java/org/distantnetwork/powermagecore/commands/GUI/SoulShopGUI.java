...
public class SoulShopGUI extends InventoryBuilder {
    public SoulShopGUI() {
        ...
        setItem(getInventory().getSize() - 9, new ItemBuilder(Material.ARROW).setName(String.format("%sBack to Main Menu", ChatColor.GRAY)).setItemFlags(ItemFlag.HIDE_ATTRIBUTES).toItem(), player -> {
            new MenuGUI().open(player);
        });
        setItem(getInventory().getSize() - 5, new ItemBuilder(Material.BARRIER).setName(String.format("%sClose Menu", org.bukkit.ChatColor.RED)).setItemFlags(ItemFlag.HIDE_ATTRIBUTES).toItem(), player -> {
